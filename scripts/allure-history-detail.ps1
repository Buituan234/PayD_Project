...
if (Test-Path $HistoryFile) {
    ...
    # Đọc file dưới dạng mảng các chuỗi thô, loại bỏ dòng trống
    ...
    if ($lines.Count -gt 0) {
        ...
        try {
            if ($lastLine.StartsWith("{") -and $lastLine.EndsWith("}")) {
                ...
                # Nén JSON thành 1 dòng duy nhất
                $lines[-1] = ($entry | ConvertTo-Json -Compress -Depth 10)
                
                # --- PHẦN SỬA ĐỔI QUAN TRỌNG: Ghi file chuẩn LF và UTF8 No BOM ---
                # Gộp các dòng lại bằng ký tự `\n` (LF) thay vì dùng Out-File mặc định của Windows
                $contentToSave = $lines -join "`n"
                
                # Sử dụng [System.IO.File] của .NET để ghi đè nhằm tránh dính BOM của PowerShell
                [System.IO.File]::WriteAllText((Resolve-Path $HistoryFile).Path, $contentToSave, (New-Object System.Text.UTF8Encoding($false)))
                
                Write-Host "History patched! URL: $ReportUrl" -ForegroundColor Green
            }
        } catch {
            Write-Host "Warning: Could not patch history.jsonl. Chi tiết: $_" -ForegroundColor Yellow
        }
    }
}
